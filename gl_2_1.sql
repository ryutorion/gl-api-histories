...
-- histories before 2.1 {{{
INSERT INTO histories
  SELECT apis.id, versions.id, 1 FROM apis, versions
    WHERE versions.name IS 'GL' AND versions.major == 2 AND versions.minor == 1;
...
UPDATE histories SET status = 0 WHERE
  histories.version_id IN (
    ...
    WHERE v.name IS 'GL' AND v.major == 2 AND v.minor == 0 AND
...
INSERT INTO histories (api_id, version_id, status)
  SELECT * FROM (
    ...
    CROSS JOIN
    (SELECT * FROM (
      SELECT id, 2 FROM versions WHERE name IS 'GL' AND major == 2 AND minor == 1
      UNION
      SELECT id, 0 FROM versions WHERE name IS 'GL' AND (major < 2 OR (major == 2 AND minor == 0)))));
-- }}}
END;
-- vim: fdm=marker
