...
-- histories before 1.5 {{{
INSERT INTO histories
  SELECT apis.id, versions.id, 1 FROM apis, versions
    WHERE versions.name IS 'GL' AND versions.major == 1 AND versions.minor == 5;
...
UPDATE histories SET status = 0 WHERE
  histories.version_id IN (
    ...
    WHERE v.name IS 'GL' AND v.major == 1 AND v.minor == 4 AND
...
INSERT INTO apis (name) VALUES ('GetQueryiv');
...
INSERT INTO histories (api_id, version_id, status)
  SELECT * FROM (
    ...
    CROSS JOIN
    (SELECT * FROM (
      SELECT id, 2 FROM versions WHERE name IS 'GL' AND major == 1 AND minor == 5
      UNION
      SELECT id, 0 FROM versions WHERE name IS 'GL' AND major == 1 AND (minor < 5))));
-- }}}
END;
-- vim: fdm=marker
