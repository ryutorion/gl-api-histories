...
UPDATE histories SET status = 0 WHERE
  histories.version_id IN (
    ...
    WHERE v.name IS 'GL' AND v.major == 3 AND v.minor == 1 AND
...
INSERT INTO histories (api_id, version_id, status)
  SELECT * FROM (
    ...
    CROSS JOIN
    (SELECT * FROM (
      SELECT id, 2 FROM versions WHERE name IS 'GL' AND major == 3 AND minor == 2
      UNION
      SELECT id, 0 FROM versions WHERE name IS 'GL' AND ((major < 3) OR (major == 3 AND minor < 2)))));
-- }}}
END;
-- vim: fdm=marker
